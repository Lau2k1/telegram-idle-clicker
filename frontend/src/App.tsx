...
function App() {
  ...
  useEffect(() => {
    ...
    // ВОЗВРАЩАЕМ ТИК РАЗ В СЕКУНДУ
    const visualInterval = setInterval(() => {
      const state = useGameStore.getState();
      const multiplier = state.isBoostActive ? 2 : 1;
      
      // Начисляем целое значение за 1 секунду
    ...
    // Синхронизация с сервером (раз в 10 тиков)
        ...
        <div className="fixed inset-0 bg-[#0f111a]/95 backdrop-blur-2xl z-[60] p-6 flex flex-col overflow-y-auto">
          ...
          <section className="mb-8">
            ...
            <div className="flex gap-4">
              ...
              <button className="flex-1 p-4 rounded-3xl bg-white/5 border border-white/5 opacity-40 flex flex-col items-center gap-2">
                <span className="text-3xl">🔴</span>
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 gap-3">
              <button onClick={() => { setActivePage('planets'); setIsMenuOpen(false); }} className={`flex items-center gap-4 p-5 rounded-3xl border ${activePage === 'planets' ? 'bg-white/10 border-blue-500' : 'bg-white/5 border-white/5'}`}>
                <div className="w-12 h-12 bg-yellow-500/20 rounded-2xl flex items-center justify-center text-2xl">💰</div>
                <div><div className="font-bold text-lg">Золотой прииск</div><div className="text-xs text-slate-400">Клик и пассивный доход</div></div>
              </button>
              <button onClick={() => { setActivePage('oil'); setIsMenuOpen(false); }} className={`flex items-center gap-4 p-5 rounded-3xl border ${activePage === 'oil' ? 'bg-white/10 border-blue-500' : 'bg-white/5 border-white/5'}`}>
                <div className="w-12 h-12 bg-blue-500/20 rounded-2xl flex items-center justify-center text-2xl">🛢️</div>
                <div><div className="font-bold text-lg">Нефтяная вышка</div><div className="text-xs text-slate-400">Добыча нефти</div></div>
              </button>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[{id:'shop',l:'Магазин',i:'🛒'}, {id:'leaderboard',l:'Топ',i:'🏆'}, {id:'stats',l:'Инфо',i:'📊'}, {id:'refinery',l:'Завод',i:'🏭'}].map(item => (
                <button key={item.id} onClick={() => { setActivePage(item.id); setIsMenuOpen(false); }} className={`flex items-center gap-3 p-4 rounded-2xl border ${activePage === item.id ? 'bg-blue-600 border-blue-400' : 'bg-white/5 border-white/5'}`}>
                  <span>{item.i}</span><span className="font-bold text-sm">{item.l}</span>
                </button>
              ))}
            </div>
          </section>
