  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/20 p-3 rounded-2xl mb-4 text-center">
        <span className="text-yellow-500 font-bold">Ваш баланс: {Math.floor(coins).toLocaleString()} 💰</span>
      </div>
      
      {upgrades.map(u => (
        <div key={u.id} className="bg-[#1a1c2c] border border-slate-700 p-4 rounded-3xl flex justify-between items-center transition-all active:scale-[0.98]">
          <div className="flex items-center gap-3">
            <span className="text-3xl bg-[#24273d] w-12 h-12 flex items-center justify-center rounded-2xl">{u.icon}</span>
            <div>
              <div className="font-bold text-white leading-tight">{u.name}</div>
              <div className="text-[10px] text-slate-400 uppercase tracking-wider">{u.desc}</div>
            </div>
          </div>
          <button 
            ...
            className={`px-5 py-2 rounded-xl font-black text-sm shadow-lg ${
              coins >= u.price ? 'bg-yellow-500 text-black shadow-yellow-500/20' : 'bg-slate-800 text-slate-500'
            }`}
          >
